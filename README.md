...
* iESBM_benchmark: data of iESBM, include datasets, features and FER results;
* Evaluator (python): code for generating evaluation results (FSR results);
* summ_example: example of output files generated by an entity summarizer to be evaluated;
* runs: output files generated by selected entity summarizers and their FSR results.
...
Your can evaluate your algorithm with iESBM according to the following process:
### Required Input Format
To evaluate your algorithm for entity summarization, please generate summaries for entities in datasets and organize the directory for summaries as follows (see [summ_example]()):
<pre>
├── ${algo_name}
│   ├── ${ds_name}
│   │   ├── ${eid}
...
where ${algo_name} is the name of your entity summarization algorithm, e.g. 'relin', 'diversum'. 
...
### Run the Evaluator
Please use the path to directory ${algo_name}, i.e. ${path_to_your_results}, as input. 
And run [iesbm_eval.py]() by the following command:
<pre>
python iesbm_eval.py -algodir ${path_to_your_results} [-feature ${feature_name} -ds_name ${ds_name} -topk ${topk}]
</pre>
where parameter <code>-feature_name</code> is optional and its values include:
 'LFoP', 'GFoP', 'GFoV', 'IoPV', 'DoP' and 'DoV'.
...
During the preprocesing, triples in summaries will be converted to triple ids, 
and output to directory <code>out_${algo_name}/algo_parsed/</code>. 
...
```
(2) Print statistical results, include the following information:
```
...
## Add New Feature
Customized features can be added to the evaluator according to the following process: 
### Triple-level feature
Firstly, compute feature score for each triple in dataset ${ds_name}, and output these information to a file named '${fname}_${ds_name}.txt' (where ${fname} represents the name of your new feature, e.g. GFoV).
In this file, with each line contains the following items (items are splitted by tab, see [GFoP_dbpedia.txt]() as example):
<pre>
${tid}, ${tscore}
</pre>
Put this file to directory <code>in/in_ds_feature/</code>.

Open <code>code/f_imp.py</code>, add a new <code>elif</code> statement to the function <code>get_feature_by_name()</code>:
<pre>
elif fname=='${fname}'
...
First, implement a new subclass of <code>f_base.Feature</code> and name tihs class as 'F_${fname}' (see class F_DoP, F_DoV in [f_imp.py]() as example).
In this class, define the method to get feature score for an entity in function <code>self._get_score_by_sscore()</code>.

Then, open <code>code/f_imp.py</code>, add a new <code>elif</code> statement to the function <code>get_feature_by_name()</code>, to return an object of the newly defined class:
<pre>
elif fname=='${fname}'
